...
@section('title', __('cities.edit'))

@section('content_header')
    <h1>{{ __('cities.edit') }}</h1>
@stop
...
@section('content')
    <form action="{{ route('admin.cities.update', $city->id) }}" method="POST">
                ...
                @foreach ($languages as $language)
                    <div class="tab-pane fade @if ($loop->first) show active @endif" id="tab_{{ $language->code }}" role="tabpanel">
                        <div class="form-group">
                            ...
                            <input type="text" name="name[{{ $language->code }}]" id="name_{{ $language->code }}" class="form-control" value="{{ old('name.' . $language->code, $city->getTranslation('name', $language->code)) }}">
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="form-check mb-3">
            <input type="checkbox" class="form-check-input" name="is_active" id="is_active" {{ $city->is_active ? 'checked' : '' }}>
