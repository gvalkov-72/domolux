...
@section('content')
    ...
    <form action="{{ route('admin.pages.update', $page->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="number" name="sort_order" id="sort_order" class="form-control" value="{{ old('sort_order', $page->sort_order) }}">
        </div>

        <div class="form-group">
            <label for="seo_title">{{ __('pages.seo_title') }}</label>
            <input type="text" name="seo_title" id="seo_title" class="form-control" value="{{ old('seo_title', $page->seo_title) }}">
        </div>

        <div class="form-group">
            <label for="seo_description">{{ __('pages.seo_description') }}</label>
            <input type="text" name="seo_description" id="seo_description" class="form-control" value="{{ old('seo_description', $page->seo_description) }}">
        </div>
            ...
            @foreach($languages as $lang)
                ...
                <div class="tab-pane fade @if($loop->first) show active @endif" id="lang-{{ $lang->code }}" role="tabpanel" aria-labelledby="tab-{{ $lang->code }}">
                    <div class="form-group">
                        ...
                        <input type="text" name="title[{{ $lang->code }}]" id="title_{{ $lang->code }}" class="form-control" value="{{ old("title.$lang->code", $trans->title ?? '') }}" required>
                    </div>

                    <div class="form-group">
                        <label for="content_{{ $lang->code }}">{{ __('pages.content') }} ({{ strtoupper($lang->code) }})</label>
                        <textarea name="content[{{ $lang->code }}]" id="content_{{ $lang->code }}" class="form-control summernote" rows="6">{{ old("content.$lang->code", $trans->content ?? '') }}</textarea>
                    </div>
                </div>
            @endforeach
        </div>
